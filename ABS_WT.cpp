...
ABS_FM::ABS_FM(unsigned char * data, int length, int block_size, int D)
{
	this->block_size = block_size;
	this->D = D;
	this->T = NULL;
	T = Getdata(data,length);
	Inittable();
}
ABS_FM::~ABS_FM()
{
...
unsigned char * ABS_FM::Getdata(unsigned char *data, int length)
{
	this->n = length;
	unsigned char * T = new unsigned char[n];
	T = data;
	//memcpy(T, data, sizeof(char) * n);
	memset(charFreq, 0, 256 * sizeof(int));
	memset(charMap, 0, 256 * sizeof(bool));
	for (int i = 0; i < n; i++)
	{
